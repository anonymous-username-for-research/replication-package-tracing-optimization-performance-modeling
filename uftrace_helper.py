...
def trace(vanilla_command, full_command, parameters, table_name, build, cwd=None, processor_number=None, skip_vanilla = False, only_vanilla = False):
    if not only_vanilla:    
        ...
        psutil.cpu_percent()
        full_process = subprocess.run(full_command, capture_output=True, cwd=cwd, timeout=200)
        ...
        vanilla_command = vanilla_command[-3:]

        start_time = time.time()
        subprocess.run(vanilla_command, capture_output=True, cwd=cwd)
        end_time = time.time()
