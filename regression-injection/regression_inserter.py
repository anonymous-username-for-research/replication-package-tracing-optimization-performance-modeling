...
def run_program(program, skip_build=False):
    ...
    if not skip_build:
        if '104' in program:
            subprocess.run(['%PATH%/SPECmpi/bin/runspec',
                    '--config=sgi-linux-ipf-intel-sgimpi.cfg', '--action=clobber', '-tune=base',
                    '--size=mtest', program], capture_output=True)
        
        if '63' in program:
            subprocess.run(['%PATH%/SPEC2017/bin/runcpu',
                    '--config=custom-gcc.cfg', '--action=clobber', '-tune=base',
                    '--size=test', program], capture_output=True)
        
        if 'freqmine' in program:
            subprocess.run(['%PATH%/PARSEC/bin/parsecmgmt',
                    '-a', 'uninstall', '-p', program], capture_output=True)
            
        if 'su2' in program:
            subprocess.run(['./ninja', '-C', 'build', 'clean'],
                           cwd='%PATH%/SU2',capture_output=True)
        
        print('Finished clobbering')

        if '104' in program:
            subprocess.run(['%PATH%/SPECmpi/bin/runspec',
                            '--config=sgi-linux-ipf-intel-sgimpi.cfg', '--action=build', '-tune=base',
                            '--size=mtest', '--iterations=1', '--noreportable', '-output_format=text', '--ranks=4',
        ...
        if 'su2' in program:
            subprocess.run(['./ninja', '-C', 'build', 'install'],
                           cwd='%PATH%/SU2',capture_output=False)
        ...
        for p in Path(input_directory).rglob(ext):
            ...
            for function in root.findall('function'):
                ...
                for designated_block in designated_blocks:
                    ...
                    # """ We want to insert srand() at the beginning of main function if the designated block requires it
                    #  The srand() function sets the initial seed for the rand() function, and should be called only once """
                    # if function_name == 'main' and blocks_specs[designated_block]['srand'] == True:
                    #     if args.program_name != '631.deepsjeng_s':
                    #         # Insert srand() before the first block element
                    #         code_block = code_blocks['srand_' + p.suffix[1:]]['code']
                    #         xml_block = ET.fromstring(code_block)
                    #         items = xml_block.findall('*')
                    #         for item in items:
                    #             block.insert(0, item)
